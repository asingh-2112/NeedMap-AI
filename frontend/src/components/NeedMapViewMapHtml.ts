// Shared HTML map generation — used by both NeedMapView.web.tsx (iframe)
// and NeedMapView.tsx (WebView fallback when native module unavailable).

import type { Need, Organization, Volunteer } from "../types/api";
import { LEAFLET_CSS, LEAFLET_JS, LEAFLET_HEAT_JS } from "./LeafletAssets";
...
<html>
  <head>
    ...
    <style>
      /* Leaflet CSS — embedded inline to avoid CDN dependency */
      ${LEAFLET_CSS}
  ...
  <body>
    <div id="map"></div>
    <script>
      // Leaflet JS — embedded inline
      ${LEAFLET_JS}
      // Leaflet.heat — embedded inline
      ${LEAFLET_HEAT_JS}
    </script>
    <script>
      ...
      const initMap = function() {
        if (typeof L === 'undefined') {
          document.body.innerHTML = '<div style="display:flex;align-items:center;justify-content:center;height:100%;color:#666;font-family:sans-serif;font-size:14px">Map library failed to load.</div>';
          return;
        }
        ...
        // Create legend control ONCE at topright
        const legendControl = L.control({ position: 'topright' });
        legendControl.onAdd = function() { const d = L.DomUtil.create('div', 'legend'); d.id = 'heatLegend'; return d; };
        legendControl.addTo(map);

        // --- Initial render ---
        if (showAdminHeatmap && needs.length > 0) {
          // Admin heatmap mode
          cachedAreaList = buildAreaList(needs);
          ...
          // Heat layer
          if (L.heatLayer && needs.length > 0 && hasUsableMapSize()) {
            try {
              heatLayer = L.heatLayer(needs.map(n => [n.lat, n.lng, priorityWeight(n)]), {
          ...
          // Need dots and entity markers
          needs.forEach(n => bounds.push([n.lat, n.lng]));
          renderNeedDots(needs);
          renderEntityMarkers();
          renderAreaBubbles();
          updateLegend(needs);

          // Zoom handler refreshes layer visibility
          ...
          // Normal mode: markers + needs + orgs
          if (currentLocation) {
            L.marker([currentLocation.lat, currentLocation.lng], { icon: currentLocationIcon(), zIndexOffset: 900 }).addTo(map)
              .bindTooltip(mapLabels.currentLocation || 'Current location', { className: 'current-location-label', direction: 'top', offset: [0,-10], permanent: true, opacity: 0.95 });
            bounds.push([currentLocation.lat, currentLocation.lng]);
          ...
          markers.forEach(m => {
            ...
            const icon = L.divIcon({ className: '', html: '<div class="entity-pin ' + (isHQ ? 'hq' : 'org') + '-pin"><span>' + (isHQ ? 'H' : 'O') + '</span></div>', iconSize: [28,28], iconAnchor: [7,24], popupAnchor: [7,-24] });
            L.marker([m.lat, m.lng], { icon: icon, zIndexOffset: isHQ ? 800 : 650 }).addTo(map)
              .bindPopup('<div class="entity-popup"><div class="entity-title">' + escapeHtml(m.label || '') + '</div></div>');
          ...
          needs.forEach(n => {
            ...
            const s = normalizeStatus(n.status);
            L.marker([n.lat, n.lng], { icon: needDotIcon(s), zIndexOffset: 760 }).addTo(map)
              .bindPopup(needPopupHtml(n));
          });

          organizations.forEach(o => {
            L.marker([o.lat, o.lng], { icon: entityIcon('org'), zIndexOffset: 650 }).addTo(map)
              .bindPopup('<div class="entity-popup"><div class="entity-title">' + escapeHtml(o.name) + '</div><div class="entity-kind">' + (mapLabels.organizationPointer || 'Organization') + '</div></div>');
            bounds.push([o.lat, o.lng]);
          });
        }
        ...
        // --- Add click handler for need detail buttons ---
        document.addEventListener('click', (e) => {
          const t = e.target && e.target.closest ? e.target.closest('[data-need-id]') : null;
          if (!t) return;
          const id = Number(t.getAttribute('data-need-id'));
